import subprocess
import os
from firewall.config_manager import load_config, save_config, get_nat_config, set_nat_config
...
def enable_nat():
    ...
    try:
        ...
        subprocess.run([
            ...
            "-o", ext_iface, "-s", int_network, "-j", "MASQUERADE"
        ], check=True)
        
        # Allow forwarding between interfaces
        subprocess.run([
            "iptables", "-A", "FORWARD",
            "-i", int_iface, "-o", ext_iface, "-j", "ACCEPT"
        ], check=True)
        
        subprocess.run([
            "iptables", "-A", "FORWARD",
            "-i", ext_iface, "-o", int_iface,
...
def configure_nat(external_interface, internal_interface, internal_network):
    ...
    try:
        # Validate interfaces
        if not all(is_interface_valid(iface) for iface in [external_interface, internal_interface]):
            raise ValueError("Invalid interface specified")
