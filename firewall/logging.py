...
def enable_logging_rules():
    ...
    rules = [
        ...
        ["iptables", "-A", "INPUT", "-m", "state", "--state", "NEW", "-j", "LOG", "--log-prefix", "NEW_CONNECTION: ", "--log-level", "4"],
        ...
        ["iptables", "-A", "INPUT", "-p", "tcp", "--tcp-flags", "ALL", "NONE", "-j", "LOG", "--log-prefix", "STEALTH_SCAN: ", "--log-level", "4"],
        ["iptables", "-A", "INPUT", "-p", "tcp", "--tcp-flags", "ALL", "ALL", "-j", "LOG", "--log-prefix", "XMAS_SCAN: ", "--log-level", "4"],
        ["iptables", "-A", "INPUT", "-f", "-j", "LOG", "--log-prefix", "FRAGMENT_PACKET: ", "--log-level", "4"]
    ]
...
def show_iptables_logs(keyword="", lines=50):
    ...
    try:
        ...
        try:
            ...
            filtered_logs = "\n".join(line for line in logs.splitlines() if any(k in line for k in keyword.split("|")))
...
def format_iptables_logs(logs):
    ...
    for line in logs.splitlines():
        if any(prefix in line for prefix in ["DROP_", "NEW_CONNECTION:", "STEALTH_SCAN:", "XMAS_SCAN:", "FRAGMENT_PACKET:", "PING_OUT:"]):
